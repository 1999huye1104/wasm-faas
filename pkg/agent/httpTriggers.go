...
type HTTPTriggerSet struct {
	...
	*mutableRouter
	...
	// funtionResultCache         map[string]chan string
}

func makeHTTPTriggerSet(logger *zap.Logger, fissionClient versioned.Interface,
	kubeClient kubernetes.Interface,redisClient *redis.Client,executor *executorClient.Client, params *tsRoundTripperParams, isDebugEnv bool, unTapServiceTimeout time.Duration, actionThrottler *throttler.Throttler) *HTTPTriggerSet {
...
func (ts *HTTPTriggerSet) getRouter(fnTimeoutMap map[types.UID]int) *mux.Router {
	...
	for i := range ts.triggers {
		...
		if fh.function.Spec.InvokeStrategy.ExecutionStrategy.ExecutorType==fv1.ExecutorTypeWasmShortA||fh.function.Spec.InvokeStrategy.ExecutionStrategy.ExecutorType==fv1.ExecutorTypeWasmShortS{
...
func (ts *HTTPTriggerSet) addFunctionHandlers() {
	ts.funcInformer.AddEventHandler(k8sCache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj interface{}, newObj interface{}) {
			...
			if oldFn.Spec.InvokeStrategy.ExecutionStrategy.ExecutorType==fv1.ExecutorTypeWasmShortA||oldFn.Spec.InvokeStrategy.ExecutionStrategy.ExecutorType==fv1.ExecutorTypeWasmShortS {
